...
class Reports_model extends CI_Model {
	...
	{
		$this->db->select('name');
		$this->db->select_sum("'". $this->db->protect_identifiers('amount', FALSE) * $this->db->protect_identifiers('quantity', FALSE) ."'", 'amount');
	...
	function getInvoiceDateRange($start_date, $end_date)
	{
		...
		$this->db->distinct();
		$this->db->select('invoices.id, dateIssued, invoice_number');
